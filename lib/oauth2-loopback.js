...
module.exports = function(app, options) {
  ...
  function userLogin(username, password, companyId, done) {
    debug('userLogin: %s', username);
    models.users.findByUsernameOrEmail(username, companyId, function(err, user) {
      if (err) {
        return done(err);
      }
      if (!user) {
  ...
  if (supportedGrantTypes.indexOf('resourceOwnerPasswordCredentials') !== -1) {
    server.exchange(oauth2Provider.exchange.password(
      function(client, username, password, companyId, scope, done) {
        ...
        userLogin(username, password, companyId, function(err, user) {
          if (err || !user) {
  ...
  handlers.authorization = [
    function(req, res, next) {
      // Check if we need to force reauthenticate.
      if (req && req.query && req.query.prompt && req.query.prompt === 'login') {
        req.session.authorize = null;
        req.session.passport = null;
        req.session.authApp = null;

        return res.redirect('/oauth/authorize?response_type=' + req.query.response_type + '&client_id=' + req.query.client_id + '&scope=' + req.query.scope + '&redirect_uri=' + req.query.redirect_uri);
      }
